...
fn ensure_log_sections(content: &str, date_str: &str) -> String {
...
#[tauri::command(rename_all = "camelCase")]
fn update_todo_entry(old_name: String, new_name: String) -> Result<(), String> {
    use std::fs;
...
#[tauri::command(rename_all = "camelCase")]
fn complete_todo_entry(todo_text: String) -> Result<(), String> {
    use std::fs;
...
#[tauri::command(rename_all = "camelCase")]
fn discard_todo_entry(todo_text: String) -> Result<(), String> {
    use std::fs;
...
#[tauri::command]
fn expand_for_dashboard(app: AppHandle) {
...
#[tauri::command]
fn collapse_from_dashboard(app: AppHandle) {
    use tauri::LogicalSize;
    use tauri::LogicalPosition;
    if let Some(window) = app.get_webview_window("overlay") {
        if let Ok(Some(monitor)) = window.primary_monitor() {
